...
import EditAnalyseModal from "./EditAnalyseModal";
      ...
      <div className="bg-white rounded-lg mb-6 p-4">
        <div className="flex justify-between items-center px-2">
          ...
          {storedUserData.role === "SOIGNANT" && (
            <button
              className="flex gap-1 items-center justify-center text-sm bg-sky-400 px-2 py-1 text-white rounded-sm"
                  ...
                  <div className="mt-2">
                    ...
                    <ul className="list-disc pl-5 text-gray-600">
                      {consultation.analyses.map((analyse) => (
                        <li
                          key={analyse.id}
                          className="cursor-pointer text-blue-500 hover:text-blue-700 duration-300"
                          onClick={() => handleEditAnalyse(analyse)} 
                        >
                          {analyse.nom}: {analyse.valeur} {analyse.unite}
                        </li>
                      ))}
                    </ul>
                  </div>
